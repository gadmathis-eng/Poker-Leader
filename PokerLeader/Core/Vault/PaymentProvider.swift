...
///
/// Nothing here touches card numbers or payment credentials, and no provider
...
/// The payout side. Cash-outs go to whichever payout provider the operator has
/// approved and the player has verified — never back out through a card sheet.
...
/// Credits demo funds while the build is in sandbox. It shows the pause a real
/// provider has and can be told to decline, so the failure path is exercised as
/// often as the happy one — but it authorises nothing, charges nothing, and the
/// token it returns is only good against the sandbox backend.
struct SandboxDepositProvider: VaultPaymentProvider {
    var displayName: String { "Demo deposit" }
    ...
    func authorize(
        ...
        if alwaysDeclines {
            throw VaultError.paymentFailed("The test deposit was declined.")
        }
...
enum VaultProviders {
    ...
    static let payment: VaultPaymentProvider = SandboxDepositProvider()
